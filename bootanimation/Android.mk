...
$(TARGET_GENERATED_BOOTANIMATION): $(SOONG_ZIP)
	...
	@rm -rf $(dir $@)
	@mkdir -p $(dir $@)
	$(hide) tar xfp vendor/droid-ng/bootanimation/bootanimation.tar -C $(INTERMEDIATES)
	...
	IMAGESCALEHEIGHT=$$(expr $$IMAGESCALEWIDTH / 4); \
	if [ "$(TARGET_BOOTANIMATION_HALF_RES)" = "true" ]; then \
	    IMAGEWIDTH="$$(expr "$$IMAGEWIDTH" / 2)"; \
	fi; \
	IMAGEHEIGHT=$$(expr $$IMAGEWIDTH / 4); \
	RESOLUTION="$$IMAGEWIDTH"x"$$IMAGEHEIGHT"; \
	for part_cnt in 0; do \
	    mkdir -p $(INTERMEDIATES)/part$$part_cnt; \
	done; \
	prebuilts/tools-lineage/${HOST_OS}-x86/bin/mogrify -bordercolor black -border 100x50 -resize $$RESOLUTION -colors 256 $(INTERMEDIATES)/*/*.png; \
	echo "$$IMAGESCALEWIDTH $$IMAGESCALEHEIGHT 30" > $(INTERMEDIATES)/desc.txt; \
	cat vendor/droid-ng/bootanimation/desc.txt >> $(INTERMEDIATES)/desc.txt
